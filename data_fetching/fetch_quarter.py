...
DATA_FOLDER = "./data/quarterly_prices"  # Folder to store monthly stock price data
MAX_RETRIES = 3  # Number of retries for failed requests
SLEEP_TIME = 12  # Sleep time between API requests (to avoid rate limits)
QUARTERLY_MONTHS = {"03", "06", "09", "12"}  # Only fetch data for March, June, September, December

# Ensure the data folder exists
if not os.path.exists(DATA_FOLDER):
    os.makedirs(DATA_FOLDER)

def fetch_quarterly_prices(stock_symbol, api_key):
    """Fetch quarterly adjusted stock price data from Alpha Vantage."""
    ...
    for attempt in range(MAX_RETRIES):
        try:
            ...
            # Extract quarterly adjusted closing prices
            ...
            quarterly_prices = {
                ...
                if date[5:7] in QUARTERLY_MONTHS  # Filter for quarterly months only
            }
    ...
    print(f"❌ Failed to fetch quarterly price data for {stock_symbol} after {MAX_RETRIES} attempts.")
    return None

def save_price_data(stock_symbol, price_data):
    """Save quarterly stock price data to a JSON file."""
    file_path = os.path.join(DATA_FOLDER, f"{stock_symbol}.json")
    with open(file_path, "w") as file:
        json.dump(price_data, file, indent=4)
    print(f"💾 Quarterly stock price data saved for {stock_symbol} in {file_path}")

def main():
    print(f"📈 Fetching quarterly stock prices for {STOCK_SYMBOL}...")

    price_data = fetch_quarterly_prices(STOCK_SYMBOL, API_KEY)
    ...
    if price_data:
        ...
        print(f"⚠️ No quarterly price data available for {STOCK_SYMBOL}")
